...
class LoginContainer extends Component<Props, State> {
    ...
    componentDidUpdate = () => {
        ...
        if (!this.state.isSetRedirectFrom && (landingURL !== "/login" || landingURL !== "/logout")) {
            this.setState({
    ...
    render() {
        ...
        return (
            <div>
                <LoginForm isLoading={isLoading} isFailure={isFailure} handler={actions.login} errorInfo={error} />
            </div>
        );
    }
}
